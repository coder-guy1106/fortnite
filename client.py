...
current_weapon = 1 
build_mode = 'wall'
GRID_SIZE = 4 # Increased slightly for better player movement inside boxes

# Structural Memory (To prevent floating pieces)
built_cells = set()
...
# ==========================================
def get_target_cell():
    """Smart Targeting: Finds the best grid cell based on where you are looking."""
    hit_info = raycast(camera.world_position, camera.forward, distance=12)
    
    if hit_info.hit:
        # If looking at a wall, snap to the cell slightly in front of that wall
        target_pos = hit_info.world_point + (hit_info.normal * 0.1)
    else:
        # If looking at the sky, build 1 grid space in front of the player
        target_pos = camera.world_position + (camera.forward * GRID_SIZE)
        
    # Convert absolute position to Grid Coordinates (cx, cy, cz)
    cx = round(target_pos.x / GRID_SIZE)
    # Ensure we don't build underground
    cy = max(0, round(target_pos.y / GRID_SIZE))
    cz = round(target_pos.z / GRID_SIZE)
    
    return cx, cy, cz

def is_supported(cx, cy, cz):
    """Checks if a block has structural integrity."""
    if cy == 0: return True # Touching the grass
    
    # Check all 6 adjacent cells
    neighbors = [
        (cx, cy-1, cz), (cx, cy+1, cz), # Below, Above
        (cx+1, cy, cz), (cx-1, cy, cz), # Right, Left
        (cx, cy, cz+1), (cx, cy, cz-1)  # Front, Back
    ]
    for n in neighbors:
        if n in built_cells: return True
    return False

def get_world_pos_and_rot(cx, cy, cz, b_type, player_rot_y):
    """Converts grid coordinates into world coordinates with Edge-Snapping."""
    x = cx * GRID_SIZE
    y = cy * GRID_SIZE
    z = cz * GRID_SIZE
    ...
    if b_type == 'floor':
        return Vec3(x, y - (GRID_SIZE/2), z), 0, (GRID_SIZE, 0.2, GRID_SIZE)
    
    elif b_type == 'ramp':
        return Vec3(x, y, z), rot_y, (GRID_SIZE, 0.1, GRID_SIZE * 1.414) # Thinner ramp
    
    elif b_type == 'wall':
        # EDGE SNAPPING: Shift the wall to the outer edge of the cell so corners meet perfectly
        if rot_y == 0:   z += GRID_SIZE/2
        elif rot_y == 90:  x += GRID_SIZE/2
        elif rot_y == 180: z -= GRID_SIZE/2
        elif rot_y == 270: x -= GRID_SIZE/2
        return Vec3(x, y, z), rot_y, (GRID_SIZE, GRID_SIZE, 0.2)
...
def update():
    ...
    if not game_started and connected_successfully:
        ...
        player.y = 5
        player.step_height = 1.2 # Critical fix: allows you to walk up ramps smoothly!
        ...
        preview_block = Entity(model='cube', color=color.rgba(0, 0, 255, 100), unlit=True)
    ...
    if game_started and ws and player:
        ...
        # Turn hologram red if unsupported
...
def input(key):
    ...
    if key == 'left mouse down':
        ...
        hit_info = raycast(camera.world_position, camera.forward, distance=150)
